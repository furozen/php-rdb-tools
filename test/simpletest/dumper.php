...
class SimpleDumper
{
    ...
    function describeDifference($first, $second, $identical = false) {
        if ($identical) {
            if (!$this->isTypeMatch($first, $second)) {
                return "with type mismatch as [" . $this->describeValue($first) . "] does not match [" . $this->describeValue($second) . "]";
            }
        }
    ...
    protected function describeStringDifference($first, $second, $identical) {
        ...
        $message .= " with [" . $this->clipString($first, 200, $position) . "] and [" . $this->clipString($second, 200, $position) . "]";
        return $message;
    }
    ...
    protected function describeIntegerDifference($first, $second, $identical) {
        ...
        return "because [" . $this->describeValue($first) . "] differs from [" . $this->describeValue($second) . "] by " . abs($first - $second);
    }
    ...
    protected function describeArrayDifference($first, $second, $identical) {
        ...
        if (!$this->isMatchingKeys($first, $second, $identical)) {
            return "as key list [" . implode(", ", array_keys($first)) . "] does not match key list [" . implode(", ", array_keys($second)) . "]";
        }
    ...
    protected function describeObjectDifference($first, $second, $identical) {
        ...
        return $this->describeArrayDifference($this->getMembers($first), $this->getMembers($second), $identical);
    }
