...
class SimpleFrameset
{
    ...
    function getFrameFocus() {
        ...
        return array_merge(array($this->getPublicNameFromIndex($this->focus)), $this->frames[$this->focus]->getFrameFocus());
    }
    ...
    function getUrlsByLabel($label) {
        if (is_integer($this->focus)) {
            return $this->tagUrlsWithFrame($this->frames[$this->focus]->getUrlsByLabel($label), $this->focus);
        }
        $urls = array();
        foreach ($this->frames as $index => $frame) {
            $urls = array_merge($urls, $this->tagUrlsWithFrame($frame->getUrlsByLabel($label), $index));
        }
        return $urls;
    }
    ...
    protected function findForm($method, $attribute) {
        if (is_integer($this->focus)) {
            return $this->findFormInFrame($this->frames[$this->focus], $this->focus, $method, $attribute);
        }
        for ($i = 0; $i < count($this->frames); $i++) {
            $form = $this->findFormInFrame($this->frames[$i], $i, $method, $attribute);
            if ($form) {
                return $form;
            }
        }
