...
class PHPUnit_TestCase extends SimpleTestCase
{
    ...
    function assertSame($first, $second, $message = "%s") {
        ...
        $message = sprintf($message, "[" . $dumper->describeValue($first) . "] and [" . $dumper->describeValue($second) . "] should reference the same object");
        return $this->assert(new TrueExpectation(), SimpleTestCompatibility::isReference($first, $second), $message);
    }
