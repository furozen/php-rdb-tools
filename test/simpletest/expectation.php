...
class NotEqualExpectation extends EqualExpectation
{
    ...
    function testMessage($compare) {
        $dumper = $this->getDumper();
        if ($this->test($compare)) {
            return "Not equal expectation passes " . $dumper->describeDifference($this->getValue(), $compare);
        }
        else {
            return "Not equal expectation fails [" . $dumper->describeValue($this->getValue()) . "] matches";
        }
    }
}
...
class WithinMarginExpectation extends SimpleExpectation
{
    ...
    protected function withinMessage($compare) {
        return "Within expectation [" . $this->dumper->describeValue($this->lower) . "] and [" . $this->dumper->describeValue($this->upper) . "]";
    }
    ...
    protected function outsideMessage($compare) {
        if ($compare > $this->upper) {
            return "Outside expectation " . $this->dumper->describeDifference($compare, $this->upper);
        }
        else {
            return "Outside expectation " . $this->dumper->describeDifference($compare, $this->lower);
        }
    }
}
...
class ReferenceExpectation
{
    ...
    function testMessage($compare) {
        if ($this->test($compare)) {
            ...
            return "Reference expectation fails " . $this->dumper->describeDifference($this->value, $compare);
        }
    }
...
class IdenticalExpectation extends EqualExpectation
{
    ...
    function testMessage($compare) {
        ...
        if ($this->test($compare)) {
            ...
            return "Identical expectation [" . $dumper->describeValue($this->getValue()) . "] fails with [" . $dumper->describeValue($compare) . "] " . $dumper->describeDifference($this->getValue(), $compare, TYPE_MATTERS);
        }
    }
}
...
class NotIdenticalExpectation extends IdenticalExpectation
{
    ...
    function testMessage($compare) {
        $dumper = $this->getDumper();
        if ($this->test($compare)) {
            return "Not identical expectation passes " . $dumper->describeDifference($this->getValue(), $compare, TYPE_MATTERS);
        }
        else {
...
class PatternExpectation extends SimpleExpectation
{
    ...
    protected function describePatternMatch($pattern, $subject) {
        ...
        $dumper = $this->getDumper();
        return "Pattern [$pattern] detected at character [$position] in [" . $dumper->describeValue($subject) . "] as [" . $matches[0] . "] in region [" . $dumper->clipString($subject, 100, $position) . "]";
    }
}
...
class IsAExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare) {
        $dumper = $this->getDumper();
        return "Value [" . $dumper->describeValue($compare) . "] should be type [" . $this->type . "]";
    }
}
...
class NotAExpectation extends IsAExpectation
{
    ...
    function testMessage($compare) {
        $dumper = $this->getDumper();
        return "Value [" . $dumper->describeValue($compare) . "] should not be type [" . $this->getType() . "]";
    }
}
...
class MemberExpectation extends IdenticalExpectation
{
     ...
     * @param object $object      Object to selectTyped.
