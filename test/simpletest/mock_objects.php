...
class ParametersExpectation extends SimpleExpectation
{
    ...
    function testMessage($parameters) {
        if ($this->test($parameters)) {
            return "Expectation of " . count($this->expected) . " arguments of [" . $this->renderArguments($this->expected) . "] is correct";
        }
        else {
    ...
    protected function describeDifference($expected, $parameters) {
        if (count($expected) != count($parameters)) {
            return "Expected " . count($expected) . " arguments of [" . $this->renderArguments($expected) . "] but got " . count($parameters) . " arguments of [" . $this->renderArguments($parameters) . "]";
        }
        $messages = array();
        for ($i = 0; $i < count($expected); $i++) {
            $comparison = $this->coerceToExpectation($expected[$i]);
            if (!$comparison->test($parameters[$i])) {
                $messages[] = "parameter " . ($i + 1) . " with [" . $comparison->overlayMessage($parameters[$i], $this->getDumper()) . "]";
            }
        }
...
class CallCountExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare) {
        return 'Expected call count for [' . $this->method . '] was [' . $this->count . '] got [' . $compare . ']';
    }
}
...
class MinimumCallCountExpectation extends SimpleExpectation
{
     ...
     */
    function testMessage($compare) {
        return 'Minimum call count for [' . $this->method . '] was [' . $this->count . '] got [' . $compare . ']';
    }
}
...
class MaximumCallCountExpectation extends SimpleExpectation
{
     ...
     */
    function testMessage($compare) {
        return 'Maximum call count for [' . $this->method . '] was [' . $this->count . '] got [' . $compare . ']';
    }
}
...
class SimpleMock
{
    ...
    function throwAt($timing, $method, $exception = false, $args = false) {
        ...
        $this->actions->registerAt($timing, $method, $args, new SimpleThrower($exception ? $exception
                                                      : new Exception()));
    }
...
class MockGenerator
{
    ...
    protected function addMethodList($methods) {
        return "    protected \$mocked_methods = array('" . implode("', '", array_map('strtolower', $methods)) . "');\n";
    }
