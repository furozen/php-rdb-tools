...
class TestOfProxyRoute extends UnitTestCase
{
    ...
    function testDefaultGet() {
        ...
        $route->setReturnReference('createSocket', $socket);
        $route->__construct(new SimpleUrl('http://a.valid.host/here.html'), new SimpleUrl('http://my-proxy'));
        $route->createConnection('GET', 15);
    }
    ...
    function testGetWithPort() {
        ...
        $route = new PartialSimpleProxyRoute();
        $route->setReturnReference('createSocket', $socket);
        $route->__construct(new SimpleUrl('http://a.valid.host:81/here.html'), new SimpleUrl('http://my-proxy:8081'));
        $route->createConnection('GET', 15);
    }
    ...
    function testGetWithParameters() {
        ...
        $route = new PartialSimpleProxyRoute();
        $route->setReturnReference('createSocket', $socket);
        $route->__construct(new SimpleUrl('http://a.valid.host/here.html?a=1&b=2'), new SimpleUrl('http://my-proxy'));
        $route->createConnection('GET', 15);
    }
    ...
    function testGetWithAuthentication() {
        ...
        $route = new PartialSimpleProxyRoute();
        $route->setReturnReference('createSocket', $socket);
        $route->__construct(new SimpleUrl('http://a.valid.host/here.html'), new SimpleUrl('http://my-proxy'), 'Me', 'Secret');
        $route->createConnection('GET', 15);
    }
}
...
class TestOfHttpPostRequest extends UnitTestCase
{
    ...
    function testContentHeadersCalculatedWithRawEntityBody() {
        ...
        $request = new SimpleHttpRequest($route, new SimplePostEncoding('raw body'));
        $this->assertIsA($request->fetch(15), 'SimpleHttpResponse');
    }
...
class TestOfHttpHeaders extends UnitTestCase
{
    ...
    function testNonStandardResponseHeader() {
        $headers = new SimpleHttpHeaders("HTTP/1.1 302 (HTTP-Version SP Status-Code CRLF)\r\n" . "Connection: close");
