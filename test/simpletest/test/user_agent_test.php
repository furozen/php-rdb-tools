...
class TestOfBrowserCookies extends UnitTestCase
{
    ...
    function testClearCookieBySettingExpiry() {
        ...
        $agent->fetchResponse(new SimpleUrl('http://this.com/this/path/page.html'), new SimpleGetEncoding());
        $this->assertIdentical($agent->getCookieValue("this.com", "this/path/", "a"), "b");
    ...
    function testAgeingAndClearing() {
        ...
        $agent->fetchResponse(new SimpleUrl('http://this.com/this/path/page.html'), new SimpleGetEncoding());
        $agent->restart("Wed, 25-Dec-02 04:24:20 GMT");
        $this->assertIdentical($agent->getCookieValue("this.com", "this/path/", "a"), "A");
        $agent->ageCookies(2);
        $agent->restart("Wed, 25-Dec-02 04:24:20 GMT");
        $this->assertIdentical($agent->getCookieValue("this.com", "this/path/", "a"), false);
    }
    ...
    function testReadingIncomingAndSettingNewCookies() {
        ...
        $this->assertEqual($agent->getBaseCookieValue("a", new SimpleUrl('http://this.com/this/path/page.html')), "AAA");
        $this->assertEqual($agent->getBaseCookieValue("b", new SimpleUrl('http://this.com/this/path/page.html')), "BBB");
    }
}
...
class TestOfHttpRedirects extends UnitTestCase
{
    ...
    function testDisabledRedirects() {
        $agent = new MockRequestUserAgent();
        $agent->returns('createHttpRequest', $this->createRedirect('stuff', 'there.html'));
    ...
    function testDoubleRedirect() {
        ...
        $agent->returnsAt(0, 'createHttpRequest', $this->createRedirect('first', 'two.html'));
        $agent->returnsAt(1, 'createHttpRequest', $this->createRedirect('second', 'three.html'));
        $agent->returnsAt(2, 'createHttpRequest', $this->createRedirect('third', 'four.html'));
    ...
    function testSuccessAfterRedirect() {
        $agent = new MockRequestUserAgent();
        $agent->returnsAt(0, 'createHttpRequest', $this->createRedirect('first', 'two.html'));
        $agent->returnsAt(1, 'createHttpRequest', $this->createRedirect('second', false));
        $agent->returnsAt(2, 'createHttpRequest', $this->createRedirect('third', 'four.html'));
        $agent->expectCallCount('createHttpRequest', 2);
        $agent->__construct();
...
class TestOfAuthorisation extends UnitTestCase
{
    ...
    function testAuthenticateHeaderAdded() {
        ...
        $agent->__construct();
        $response = $agent->fetchResponse(new SimpleUrl('http://test:secret@this.host'), new SimpleGetEncoding());
    }
}
